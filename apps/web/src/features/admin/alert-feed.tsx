...
export function AlertFeed() {
  ...
  const { data: alerts = [], isLoading, isError } = useQuery<Alert[]>({
    ...
    queryFn: () => apiGet<Alert[]>("/api/admin/alerts"),
  });

  const resolveMutation = useMutation({
    mutationFn: (id: string) => apiPatch<void>(`/api/admin/alerts/${id}/resolve`, {}),
  ...
  return (
    <AppShell title="Alert Feed">
      <div className="grid gap-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          {(["critical","warning","info"] as Severity[]).map(sev=>(
            <button key={sev} onClick={()=>setFilterSev(filterSev===sev?"ALL":sev)}
              ...
              <p className="text-3xl font-light mt-1">{isLoading ? "—" : counts[sev]}</p>
        ...
        {isError && <p className="text-sm text-[#8B2F2F]">Failed to load alerts. Check your connection.</p>}

        {/* Alerts */}
        <div className="grid gap-2">
          {isLoading
            ? Array.from({length:3}).map((_,i)=>(
                <div key={i} className="rounded border-l-4 border-border p-4 animate-pulse">
                  <div className="h-3 w-48 rounded bg-cream-200 mb-2" />
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  {!alert.resolved && (
                    <button
                      ...
                      className="text-xs text-[#2F567A] hover:underline shrink-0 disabled:opacity-50">
                      Mark Resolved
                    </button>
                  )}
                </div>
